const MultiSigWallet = artifacts.require('./ReindeerFund.sol')
const web3 = MultiSigWallet.web3
const TestToken = artifacts.require('./utils/TestToken.sol')
const TestCalls = artifacts.require('./utils/TestCalls')
...
const utils = require('./utils/multisigwalletutil')
...
contract('MultiSigWallet', (accounts) => {
    ...
    */
    ...
    it('callReceive1uint', async() => {
        ...
        const num=await callsInstance.lastMsgDataLength()
        assert.equal(
            32 + 4 + 28,
            num.toNumber()
        )
    ...
    it('callReceive1bytes', async() => {
        ...
        const num=await callsInstance.lastMsgDataLength()
        assert.equal(
            868+ 28, // 800 bytes data + 32 bytes offset + 32 bytes data length + 4 bytes method signature + 28
            num.toNumber()
        )
        assert.equal(
            10,
